...
int vgpu_bar_probe(struct pci_dev *pdev, struct mem_region*bar,
                   unsigned bar_num)
{
    ...
    dev_info(dev, "%s bar start = 0x%lx, len = %lu\n",
             ...
             (unsigned long)bar->len);

    return 0;
}

static int vgpu_probe(struct pci_dev *pdev,
                      const struct pci_device_id *pdev_id)
{
    ...
    bar_reg.name = "reg";
    if (vgpu_bar_probe(pdev, &bar_reg, 2) < 0)
        goto pci_release;

    /* BAR4: data store ram */
    vgpu_dev.shm = &bar_shm;
    bar_shm.name = "shm";
    if (vgpu_bar_probe(pdev, &bar_shm, 4) < 0)
        goto pci_release;
    ...
    if (DRM_READ8(vgpu_dev.reg, REG_ZERO_COPY)) {
        ...
        if (vgpu_bar_probe(pdev, &bar_zcopy, 5) < 0)
            goto pci_release;
    }
    else {
        vgpu_dev.zcopy = NULL;
...
static int vgpu_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    case ALLOC_TYPE_SHM:
        ...
        offset += (unsigned long)(vgpu_dev.shm->start +
...
static long vgpu_ioctl(struct file *filp, unsigned int cmd,
                       unsigned long arg)
{
    int r = -EINVAL;
    struct app_info *app_info = filp->private_data;
...
static int vgpu_reboot_callback(struct notifier_block *self,
...
static int __init vgpu_init(void)
{
    ...
    /* create udev node */
    {
        dev_t devt;
        devt = MKDEV(vgpu_drv_major, VGPU_DRIVER_MINOR);
        if (!(device_create(vgpu_class, NULL, devt, NULL,
                        VGPU_DEV_NAME "%d", VGPU_DRIVER_MINOR))) {
...
unregister_dev:
    unregister_chrdev_region(MKDEV(vgpu_drv_major, VGPU_DRIVER_MINOR),
                             VGPU_DEVS_NUM);
