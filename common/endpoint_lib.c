...
void* nw_handle_pool_deref(struct nw_handle_pool *pool,
    ...
    if (id == NULL || pool == NULL || !is_handle(id))
        return (void*)id;
    pthread_mutex_lock(&pool->lock);
...
void nw_handle_pool_assign_handle(struct nw_handle_pool *pool,
                                  const void *id, const void *handle) {
    ...
    // Do not check if there is already an ID with this handle since that could happen
    // legitimately (e.g., if a handle is reused by the underlying library in the
    // replay, but not in the original execution).
...
static void _ava_extract_traverse(gpointer root, struct ava_extraction_state_t* state) {
    ...
    if (added) {
        ...
        if (metadata->dependencies)
            g_ptr_array_foreach(metadata->dependencies, (GFunc) _ava_extract_traverse, state);
...
static void _ava_transfer_command(struct command_channel *output_chan, struct command_channel_log *log_chan, size_t offset) {
    uint32_t flags;
...
static void _ava_extract_explicit(gpointer obj, gpointer value, struct ava_extraction_state_t* state) {
    ...
    struct ava_replay_command_t *cmd = (struct ava_replay_command_t *)
            command_channel_new_command(state->output_chan, sizeof(struct ava_replay_command_t), chan_len);
...
static void _ava_transfer_command_in_pair(struct command_channel *output_chan, struct command_channel_log *log_chan, size_t offset_a, size_t offset_b) {
    ...
    if (flags_a != 1 && flags_b != 1) {
        ...
        struct ava_replay_command_pair_t *combine = (struct ava_replay_command_pair_t *)
            command_channel_new_command(output_chan, sizeof(struct ava_replay_command_pair_t), chan_len);

        combine->base.api_id = COMMAND_HANDLER_API;
        combine->base.command_id = COMMAND_HANDLER_RECORDED_PAIR;
        combine->call_cmd = command_channel_attach_buffer(output_chan, (struct command_base *)combine, call_cmd, call_cmd->command_size + call_cmd->region_size);
        combine->ret_cmd = command_channel_attach_buffer(output_chan, (struct command_base *)combine, ret_cmd, ret_cmd->command_size + ret_cmd->region_size);
...
//  AMP: I think we should be able to replace the original function with this one. The paired replay commands should work in all cases.
...
static struct ava_coupled_record_t *ava_get_coupled_record_unlocked(struct ava_endpoint *endpoint, const void *coupled)
{
    ...
    if (migration_call_id_str != NULL) {
        if (*migration_call_id_str == 'r') {
            ...
            assert(RAND_MAX / limit > 100 &&
                   "Limit is large enough that migration call select bias may be more than 1%. Make AMP fix it.");
            struct timeval tv;
...
#ifdef AVA_BENCHMARKING_MIGRATE
    printf
        ("INFO: Final call count = %lld\n(Set AVA_MIGRATION_CALL_ID=r%lld to choose a random call from this count. "
        "Note the 'r' before the number. Omit the 'r' to always migrate at exactly the specified call id.)\n",
...
void *ava_shadow_buffer_get(struct ava_endpoint *endpoint, void *id, size_t size,
                            enum ava_lifetime_t lifetime, void *lifetime_coupled, ava_allocator alloc, ava_deallocator dealloc);

void *ava_shadow_buffer_new_shadow(struct ava_endpoint *endpoint, void *id, size_t size,
                                   enum ava_lifetime_t lifetime, void *lifetime_coupled, ava_allocator alloc, ava_deallocator dealloc);

void ava_shadow_buffer_new_solid(struct ava_endpoint *endpoint, void *local, size_t size,
...
void *ava_shadow_buffer_get_unlocked(struct ava_endpoint *endpoint, void *id, size_t size,
...
// Coupled lifetimes will be handled by registering the returned buffer as coupled.
// Manual lifetimes will have an deallocate call.
// Static lifetimes will never end and will never be deallocated.
                                    ...
                                    ava_allocator alloc, ava_deallocator dealloc)
{
    ...
    assert(record != NULL && "Only call ava_shadow_buffer_get directly when the buffer is known to already exist.");
    assert(record->id == id);
...
struct ava_shadow_record_t *ava_shadow_buffer_new_record_unlocked(
        struct ava_endpoint *endpoint, void *id, size_t size,
...
void
ava_shadow_buffer_free_record_unlocked(const struct ava_endpoint *endpoint, const struct ava_shadow_record_t *record)
{
    DEBUG_PRINT("shadow buffer: Deallocating shadow buffer: id=%#lx, local=%#lx, size=%ld\n",
                                            ...
                                            ava_deallocator dealloc)
{
...

    {
        ...
        if (record != NULL) {
            ...
            DEBUG_PRINT("Create shadow buffer: Existing shadow buffer: id=%#lx, local=%#lx, size=%ld; new: size=%ld\n",
    ...
    DEBUG_PRINT("shadow buffer: Creating shadow buffer: id=%#lx, local=%#lx, size=%ld\n",
                (long int)record->id, (long int)record->local, (long int)record->size);
    if (lifetime == AVA_MANUAL)
        lifetime_coupled = record->local;
...
void ava_shadow_buffer_free_coupled_unlocked(struct ava_endpoint *endpoint, void *obj) {
    ...
    if (metadata != NULL && metadata->coupled_shadow_buffers != NULL) {
        for (size_t i = 0; i < metadata->coupled_shadow_buffers->len; i++) {
            ava_shadow_buffer_free_unlocked(endpoint,
                                            ((struct ava_shadow_record_t *) g_ptr_array_index(
                                                    metadata->coupled_shadow_buffers, i))->local);
        }
...
void *
ava_shadow_buffer_attach_buffer(struct ava_endpoint *endpoint, struct command_channel *chan, struct command_base *cmd,
                                ...
                                struct ava_buffer_header_t *header)
{
    ...
    header->has_data = 1;
    header->size = size;
    // TODO: This relies on the fact that buffers are allocated contiguously in some larger space which is available to the command receiver.
...
void *
ava_shadow_buffer_attach_buffer_without_data(struct ava_endpoint *endpoint, struct command_channel *chan,
...
void *
ava_shadow_buffer_get_buffer(struct ava_endpoint *endpoint, struct command_channel *chan, struct command_base *cmd,
                             void *offset, enum ava_lifetime_t lifetime, void *lifetime_coupled, size_t *size_out,
                             ava_allocator alloc, ava_deallocator dealloc)
{
    assert(lifetime != AVA_CALL);
    struct ava_buffer_header_t *header = command_channel_get_buffer(chan, cmd,
                                                                    offset - sizeof(struct ava_buffer_header_t));
    // void *data = ((void *) header) + sizeof(struct ava_buffer_header_t);
    if (size_out)
        *size_out = header->size;
    void *shadow = ava_shadow_buffer_new_shadow(endpoint, header->id, header->size, lifetime, lifetime_coupled, alloc,
                                                dealloc);
