...
size_t command_channel_log_buffer_size(const struct command_channel* chan, size_t size) {
    return size;
}

// TODO: Currently this implementation can only write to files because it uses seek a lot.
...
/**
 * Create a new command for sending on this channel. This will allocate space in the log for this command.
 *
 * @param c The command_channel_log.
 * @param command_struct_size The size of the command struct to allocate.
 * @param data_region_size The combined size of buffers which will be attached to this command.
 * @return The new command.
 */
struct command_base* command_channel_log_new_command(struct command_channel* c, size_t command_struct_size, size_t data_region_size) {
...
/**
 * Attach a buffer to the command by writing the data to the output file directly.
 * @param c The command_channel_log.
 ...
 */
void* command_channel_log_attach_buffer(struct command_channel* c, struct command_base* cmd, void* buffer, size_t size) {
    struct command_channel_log *chan = (struct command_channel_log *)c;
    struct command_private *priv = (struct command_private *)cmd->reserved_area;
 ...
 *
 */
ssize_t command_channel_log_transfer_command(struct command_channel_log* c, const struct command_channel* source, const struct command_base* cmd)
{
    struct command_channel_log *chan = (struct command_channel_log *)c;
 ...
 */
struct command_base *command_channel_log_load_command(struct command_channel_log* chan, ssize_t offset, uint32_t *flags) {
    struct record_command_metadata metadata;
    struct command_base *cmd;

    if (offset < 0)
        offset = chan->read_offset;
    ...
    // PERFORMANCE: If this read turns out to be huge and a problem we could mmap instead.
 ...
 * is called on `cmd`.
 */
void* command_channel_load_get_buffer(const struct command_channel* chan, const struct command_base* cmd, void* buffer_id) {
