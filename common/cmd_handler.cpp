...
struct command_handler_t {
  void (*replay)(struct command_channel *__chan,
                 struct nw_handle_pool *handle_pool,
                 struct command_channel *__log,
                 const struct command_base *__call_cmd,
                 const struct command_base *__ret_cmd);
  void (*handle)(struct command_channel *__chan,
                 struct nw_handle_pool *handle_pool,
                 struct command_channel *__log,
...
static int handle_command(struct command_channel *chan,
                          struct nw_handle_pool *handle_pool,
...
EXPORTED_WEAKLY void register_command_handler(
  ...
  DEBUG_PRINT(
      "Registering API command handler for API id %d: handler at 0x%lx\n",
      api_id, (uintptr_t)handle);
  struct command_handler_t *api = &nw_apis[api_id];
  assert(api->handle == NULL &&
         "Only one handler can be registered for each API id");
  api->handle = handle;
  api->print = print;
  api->replay = replay;
}

EXPORTED_WEAKLY void print_command(FILE *file,
                                   const struct command_channel *chan,
...
void handle_command_and_notify(struct command_channel *chan,
                               struct command_base *cmd) {
...
EXPORTED_WEAKLY void init_command_handler(
    struct command_channel *(*channel_create)()) {
  pthread_mutex_lock(&nw_handler_lock);
  if (!init_command_handler_executed) {
    nw_global_command_channel = channel_create();
    pthread_create(&nw_handler_thread, NULL, dispatch_thread_impl,
                   (void *)nw_global_command_channel);
...
static void replay_command(struct command_channel *chan,
                           struct nw_handle_pool *handle_pool,
                           struct command_channel *log,
                           const struct command_base *call_cmd,
...
void internal_api_handler(struct command_channel *chan,
  ...
  switch (cmd->command_id) {
    ...
    case COMMAND_START_MIGRATION: {
      ...
      transfer_chan = (struct command_channel *)command_channel_log_new(
          nw_worker_id + 1000);
      ...
      ava_extract_objects(
          ...
          nw_handle_pool_get_live_handles(nw_global_handle_pool));
      {
        struct command_base *log_end = command_channel_new_command(
            transfer_chan, sizeof(struct command_base), 0);
      ...
      struct command_channel_log *replay_log =
          command_channel_log_new(nw_worker_id);
      ...
      while (1) {
        ...
        struct command_base *call_cmd =
            command_channel_receive_command(transfer_chan);
        if (call_cmd->api_id == COMMAND_HANDLER_API) {
          ...
          handle_command(transfer_chan, replay_handle_pool,
                         (struct command_channel *)nw_record_command_channel,
                         call_cmd);
        } else {
          ...
          // Replay the commands.
          replay_command(transfer_chan, replay_handle_pool,
                         (struct command_channel *)replay_log, call_cmd,
                         ret_cmd);
    ...
    case COMMAND_HANDLER_REPLACE_EXPLICIT_STATE: {
      ava_handle_replace_explicit_state(chan, handle_pool,
    ...
    case COMMAND_START_LIVE_MIGRATION: {
      ...
      {
        struct command_base *log_init = command_channel_new_command(
            transfer_chan, sizeof(struct command_base), 0);
    ...
    case COMMAND_HANDLER_RECORDED_PAIR: {
      struct ava_replay_command_pair_t *combine =
          (struct ava_replay_command_pair_t *)cmd;
      struct command_base *call_cmd =
          (struct command_base *)command_channel_get_buffer(
      ...
      printf("replay command <%ld, %lx>\n", call_cmd->command_id,
             call_cmd->region_size);

      // Replay the commands.
      replay_command(chan, nw_global_handle_pool,
                     (struct command_channel *)nw_record_command_channel,
                     call_cmd, ret_cmd);
      break;
    }
...
EXPORTED_WEAKLY void init_internal_command_handler() {
  ...
  register_command_handler(COMMAND_HANDLER_API, internal_api_handler, NULL,
                           NULL);
}
