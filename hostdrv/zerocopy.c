...
static int zcopy_release(struct inode *inode, struct file *filp)
{
    return 0;
}
...
static long zcopy_ioctl(struct file *filp, unsigned int cmd,
                       unsigned long arg)
{
    int r = -EINVAL;
    uintptr_t phys;
...
static int __init zcopy_init(void)
{
    ...
    if (!(dev_node = device_create(dev_class, NULL,
                    MKDEV(VGPU_ZCOPY_DRIVER_MAJOR, VGPU_ZCOPY_DRIVER_MINOR), NULL, DEVICE_NAME))) {
    ...
    if (!zcopy_va) {
        ...
        pr_info("[ava-zcopy] zero_copy region pa = 0x%lx, va = 0x%lx\n",
                (uintptr_t)virt_to_phys(zcopy_va), (uintptr_t)zcopy_va);
    }
