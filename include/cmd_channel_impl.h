...
struct command_channel_vtable {
    ...
    void (*command_channel_transfer_command)(struct command_channel* chan, const struct command_channel* target, const struct command_base* cmd);
    struct command_base* (*command_channel_receive_command)(struct command_channel* chan);
    void* (*command_channel_get_buffer)(const struct command_channel* chan, const struct command_base* cmd, void* buffer_id);
...
#define __COMMAND_CHANNEL_VTABLE_CHECK_METHOD(vtable, n) assert(vtable.n != NULL && (#vtable " is missing value for " #n))
