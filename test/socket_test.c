...
#define CHECK(p) ({ typeof(p) __r = p; if(__r != 0) perror(#p); __r; })
#define CHECKN(p) ({ typeof(p) __r = p; if(__r == -1) perror(#p); __r; })
...
//            printf("Warmup complete.\n");
        ...
        do_send(fd, i);
        if (USE_NODELAY_AS_FLUSH)
            socket_flush(fd);
        if (is_server) {
            do_recv(fd, i);
        }

        if (i > 16 && i % 32 == 0 && probe_time_end(&start) > 30000)
            break;
    }
    ...
    if (is_server) {
        ...
        printf("%s%s%s, ", USE_NODELAY ? "USE_NODELAY" : "", USE_NODELAY_AS_FLUSH ? "USE_NODELAY_AS_FLUSH" : "", USE_CORK ? "USE_CORK" : "");
