...
#define ck_assert_int_buffer_elements_eq(buffer, size, V) for(int __index=0; __index<size; __index++) ck_assert_int_eq(V, buffer[__index])
...
START_TEST(buffers_struct)
    ...
    for(int i=0; i < buf.size; i++)
        ck_assert_int_eq(i, buf.buffer[i]);
END_TEST
