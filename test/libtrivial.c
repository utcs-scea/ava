...
void function3b(int session, void *userdata) {
    callbacks[session](userdata, "This is an error");
}
...
void *
special_alloc(size_t size)
{
    return malloc(size);
}

void
special_free(void *ptr)
{
    free(ptr);
}
...
int use_handle(struct handle_t *buffer)
{
    return buffer->value;
}
...
{
    if (handle->returned_buffer == NULL)
        handle->returned_buffer = calloc(1, sizeof(struct struct_buffer_t));
...
#define MILLIS_TO_TIMEVAL(t) { (time_t)t / 1000, ((suseconds_t)t % 1000) * 1000}
