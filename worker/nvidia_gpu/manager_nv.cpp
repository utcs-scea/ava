...
inline int get_worker_port(int id) {
    return worker_port_base + id;
}

int get_gpu_id_from_uuid(const std::vector<std::string> &visible_cuda_device_uuid, const char *gpu_uuid)
{
    int gpu_id;
    auto itr = std::find(visible_cuda_device_uuid.begin(), visible_cuda_device_uuid.end(),
                        std::string(gpu_uuid));
        ...
        int worker_port_min, int worker_port_max)
{
    ...
    char *const envp_list[] = {
        (char*)visible_cuda_device[gpu_id].c_str(),
        (char*)"AVA_CHANNEL=TCP",
        NULL
    };
    if (execvpe("./worker", argv_list, envp_list) < 0) {
        perror("execv worker");
    }
}
        ...
        case NW_NEW_APPLICATION:
            ...
            */
            ...
            if (worker_pool_enabled && assigned_worker_port != 0) {
                ...
                if (idle_workers[gpu_id].size() > WORKER_POOL_SIZE - 1)
                    break;
...
int main(int argc, char *argv[])
{
    ...
    /* Worker information */
    Workers *idle_workers = new Workers[total_gpu_num]; /* Each GPU has a pool of pre-spawned workers */
