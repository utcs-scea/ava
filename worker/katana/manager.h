...
public:
  ...
  friend std::ostream& operator<<(std::ostream& out, ServerAddress const& sa) {
...
public:
  ...
  void AddIdleWorker(const ServerAddress& address) {
    idle_workers_.Enqueue(address);
  }
  ...
  void AddBusyWorker(const ServerAddress& address, uint64_t used_memory) {
  ...
  void UnlockedSort() {
    ...
    std::sort(gpu_list_.begin(), gpu_list_.end(),
              [](const gpu_list_entry_t& a, const gpu_list_entry_t& b) -> bool {
                if (a->worker_counter_ == b->worker_counter_)
