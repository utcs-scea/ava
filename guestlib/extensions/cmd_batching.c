...
#define batch_array_index(array, index_) \
    ((struct command_base *)g_ptr_array_index(array, index_))
...
static void batch_emit(GAsyncQueue *active_cmds)
{
    ...
    size_t __total_buffer_size = 0;
    {
        ...
        while ((cmd = g_async_queue_try_pop(active_cmds))) {
            ...
            __total_buffer_size += ((struct command_base *)cmd)->command_size +
                ((struct command_base *)cmd)->region_size;
        }
    }
    ...
    void *__command_buffer = malloc(__total_buffer_size);
    {
        ...
        while ((cmd = (struct command_base *)g_queue_pop_tail(queue))) {
            memcpy((char *)__command_buffer + offset,
                    (void *)cmd,
 ...
 * the API server.
 */
EXPORTED_WEAKLY void batch_insert_command(struct command_batch *cmd_batch,
                                        struct command_base *cmd,
                                        struct command_channel *chan,
                                        int is_async)
{
    ...
    while (cmd_batch->running) {
        ...
        if (wrap && !wrap->is_async && g_async_queue_length(cmd_batch->active_cmds) == 0) {
            DEBUG_PRINT("Emit a synchronous command %ld, thread id %lx->%lx\n",
                    wrap->cmd->command_id, wrap->cmd->thread_id, thread_id);
        ...
        if (wrap)
            g_async_queue_push(cmd_batch->active_cmds, (gpointer)wrap->cmd);
...
{
    cmd_batch->running = 0;
    if (pthread_cancel(cmd_batch->process_thread))
