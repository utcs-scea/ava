...
std::vector<struct command_channel*> command_channel_socket_tcp_guest_new() {
  ...
  boost::split(manager_addr, guestconfig::config->manager_address_,
               boost::is_any_of(":-/ "));
  BOOST_ASSERT_MSG(manager_addr.size() == 2,
                   "Invalid API server manager address");
  boost::asio::connect(manager_sock,
                       resolver.resolve({manager_addr[0], manager_addr[1]}));
  ...
  boost::asio::write(manager_sock,
                     boost::asio::buffer(&request_length, sizeof(uint32_t)));
  boost::asio::write(manager_sock,
                     boost::asio::buffer(request_buf.data(), request_length));

  // De-serialize API server addresses
  uint32_t reply_length;
  boost::asio::read(manager_sock,
                    boost::asio::buffer(&reply_length, sizeof(uint32_t)));
  std::vector<unsigned char> reply_buf(reply_length);
  zpp::serializer::memory_input_archive in(reply_buf);
  boost::asio::read(manager_sock,
                    boost::asio::buffer(reply_buf.data(), reply_length));
  ...
  for (const auto& wa : worker_address) {
    ...
    parseServerAddress(wa.c_str(), &worker_server_info, worker_name,
                       &worker_port);
    ...
    while (connect_ret) {
      ...
      if ((uint64_t)std::chrono::duration_cast<std::chrono::milliseconds>(
              connect_checkpoint - connect_start)
              .count() > guestconfig::config->connect_timeout_) {
        std::cerr << "Connection to " << worker_address[0] << " timeout"
                  << std::endl;
        goto error;
      }
    }
