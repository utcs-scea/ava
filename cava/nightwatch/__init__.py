...
def _build_assert(ErrorType):
    def _assert(pred, *description, loc=None, kind=ice):
        if not pred:
            raise ErrorType(kind, *(_parse_state.descriptions + list(description)), loc=loc or _parse_state.locations[-1])
    return _assert
...
def _build_expects(ErrorType):
    def _expects(pred, *description, loc=None, kind=warning):
        if not pred:
            ErrorType(kind, *(_parse_state.descriptions + list(description)), loc=loc or _parse_state.locations[-1]).report()
    return _expects
