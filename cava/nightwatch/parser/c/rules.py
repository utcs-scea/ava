...
class TypeRule(Rule):
    def matches(self, ct, data):
        return isinstance(ct, cindex.Type) and self._matches_type(ct, data) or \
               isinstance(ct, cindex.Cursor) and ct.kind != CursorKind.FUNCTION_DECL and \
               self._matches_type(ct.type, data)
...
class ConstPointerTypes(PointerTypes):
    def _matches_type(self, ct, data):
        return super()._matches_type(ct, data) and (ct.is_const_qualified() or
                                                    ct.get_pointee().is_const_qualified())


class NonconstPointerTypes(PointerTypes):
    def _matches_type(self, ct, data):
        return super()._matches_type(ct, data) and not (ct.is_const_qualified() or
                                                        ct.get_pointee().is_const_qualified())


class Types(TypeRule):
    def __init__(self, tpe, annotations):
        super().__init__(annotations)
        self.type = tpe
        self.type_str = model.Type._drop_const(self.type.spelling)

    def _is_correct_type(self, ct):
