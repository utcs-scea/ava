...
void
TF_SetStatus(TF_Status* s,
             TF_Code code,
             const char* msg)
{
    ava_argument(msg) {
        ava_in; ava_buffer(strlen(msg));
    }
}
...
// Buffer
TF_Buffer*
TF_NewBufferFromString(const void* proto,
                       size_t proto_len)
{
    ava_argument(proto) {
        ava_in; ava_buffer(proto_len);
    }
    ava_return_value {
        ava_allocates;
        ava_lifetime_manual;
...
void
TF_DeleteBuffer(TF_Buffer *buf)
{
#warning need to find the matched buffer
    ava_unsupported;
    ava_argument(buf) {
        ava_in; ava_deallocates;
    }
}
             ...
             void* deallocator_arg)
{
    ...
    ava_argument(data) {
        ava_in; ava_buffer(len);
    }
    ava_argument(deallocator) {
        ava_callback(TF_deallocator);
    }
    ava_argument(deallocator_arg) {
        ava_userdata;
    }
    ava_return_value { ava_allocates; ava_handle; }
...
{
    ava_argument(tensor) ava_deallocates;
}
...
int64_t
TF_Dim(const TF_Tensor* tensor,
       int dim_index);

size_t
TF_TensorByteSize(const TF_Tensor*);

void*
TF_TensorData(const TF_Tensor* tensor)
{
    ava_return_value {
        ava_lifetime_coupled(tensor);
    };
}

int64_t
TF_TensorElementCount(const TF_Tensor* tensor)
{
    ava_unsupported;
}
...
// String
size_t
TF_StringDecode(const char* src,
                size_t src_len,
                const char** dst,
                size_t* dst_len,
                TF_Status* status)
...
size_t
TF_StringEncodedSize(size_t len);

// Session options
TF_SessionOptions*
TF_NewSessionOptions(void)
{
    ava_return_value { ava_allocates; ava_handle; }
}

void
TF_SetTarget(TF_SessionOptions* options,
             const char* target)
{
    ava_argument(target) {
        ava_in; ava_buffer(strlen(target));
    }
}

void
TF_SetConfig(TF_SessionOptions* options,
             const void* proto,
             size_t proto_len,
             TF_Status* status)
{
...
void
TF_DeleteSessionOptions(TF_SessionOptions* options)
{
    ava_argument(options) ava_deallocates;
}
...
{
    ava_argument(graph) ava_deallocates;
}

void
TF_GraphSetTensorShape(TF_Graph* graph,
                       TF_Output output,
                       const int64_t* dims,
                       const int num_dims,
                       TF_Status* status)
...
TF_NewOperation(TF_Graph* graph,
                const char* op_type,
                const char* oper_name)
{
    ava_argument(op_type) {
        ava_in; ava_buffer(strlen(op_type));
    }
    ava_argument(oper_name) {
        ava_in; ava_buffer(strlen(oper_name));
    }
    ava_return_value { ava_allocates; ava_handle; }
}

void
TF_SetDevice(TF_OperationDescription* desc,
             const char* device)
{
    ava_argument(device) {
        ava_in; ava_buffer(strlen(device));
    }
}

TF_Operation*
TF_FinishOperation(TF_OperationDescription* desc,
                   TF_Status* status)
{
...
int
TF_OperationNumOutputs(TF_Operation* oper);

int
TF_OperationNumInputs(TF_Operation* oper);

TF_Operation*
TF_GraphOperationByName(TF_Graph* graph,
                        const char* oper_name)
{
    ava_argument(oper_name) {
        ava_in; ava_buffer(strlen(oper_name) + 1);
    }
    ava_return_value ava_handle;
}

TF_Operation*
TF_GraphNextOperation(TF_Graph* graph,
                      size_t* pos)
{
    ava_argument(pos) {
        ava_out; ava_buffer(1);
...
TF_NewImportGraphDefOptions(void)
{
    ava_return_value { ava_allocates; ava_handle; }
}
...
{
    ava_argument(opts) ava_deallocates;
}

void
TF_GraphImportGraphDef(TF_Graph* graph,
...
void
TF_CloseSession(TF_Session* sess,
                TF_Status* status)
{
    ava_argument(status) {
        ava_handle;
...
void
TF_DeleteSession(TF_Session* sess,
                 TF_Status* status)
{
              ...
              TF_Status* status)
{
    ...
    ava_argument(inputs) {
        ava_in; ava_buffer(ninputs);
    }
    ava_argument(input_values) {
        ava_in; ava_buffer(ninputs);
    }
    ava_argument(outputs) {
        ava_in; ava_buffer(noutputs);
    }
    ava_argument(output_values) {
        ava_out; ava_buffer(noutputs);
    }
    ava_argument(target_opers) {
        ava_in; ava_buffer(ntargets);
