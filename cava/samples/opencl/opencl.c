                          ...
                          cl_int *       errcode_ret)
{
    ava_argument(strings) {
        ava_in;
        ava_buffer(count);
        ava_element ava_buffer( (lengths != NULL && lengths[ava_index]) ? lengths[ava_index] : strlen(strings[ava_index])+1 );
    }
                ...
                cl_int *                      errcode_ret)
{
    ...
    ava_argument(properties) {
        ava_in; ava_buffer(clCreateContext_properties_size(properties));
        ava_element {
    ...
    ava_return_value { ava_allocates; ava_object_record; }
}
...
{
    ava_async;
    ava_argument(context) { ava_deallocates; ava_object_record; }
}
...
{
    ava_async;
    ava_argument(command_queue) { ava_deallocates; ava_object_record; }
}
               ...
               cl_int *     errcode_ret)
{
    ava_argument(kernel_name) {
        ava_in; ava_buffer(strlen(kernel_name)+1);
    }
    ava_argument(errcode_ret) {
        ava_out; ava_buffer(1);
    }
...
{
    ava_argument(kernel) { ava_deallocates; ava_object_record; }
}
...
ava_utility void* cl_mem_extract(void *obj, size_t *length) {
    ...
    clGetContextInfo(ava_metadata(obj)->context, CL_CONTEXT_DEVICES,
            sizeof(cl_device_id) * 10, ava_metadata(obj)->devices, NULL);

	cl_command_queue cmd_q = clCreateCommandQueue(ava_metadata(obj)->context,
            ava_metadata(obj)->devices[0], NULL, NULL);
