...
ava_utility void QATAVAFree(void *ptr)
{
    qaeMemFreeNUMA(&ptr);
}
...
ava_type(CpaFlatBuffer) {
    ...
    ava_field(pData) {
        ...
        ava_lifetime_static;
        // AMP: If ava_{in,out}put were value annotations they should be placed here in a sense. However they would actually need to be added at the call. Which is pain, but #define will help.
    }
}
...
ava_callback_decl
void cpa_callbackFn(void *userdata, CpaStatus status) {
    ...
    void (*callbackFn) (void *, CpaStatus))
{
    ...
    ava_argument(callbackFn) {
        ava_callback_registration(cpa_callbackFn);
    }
    ...
    void *callbackTag)
{
    ...
    */
    ...
    ava_consumes_resource(qat_throughput,
            calc_CpaBufferList_size(pSrcBuff) / used_time);
    */
    ...
    void *callbackTag)
{
    ...
    // AMP: In async mode, pDestBuff needs to be stored in ava_metadata(callbackTag) and transported as an implicit argument of the callback. Otherwise the result data will never be copied back to the guest.
}
