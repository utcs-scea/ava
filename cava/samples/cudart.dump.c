...
ava_utility void __helper_dump_fatbin(void *fatCubin,
    ...
    sprintf(fatbin_filename, "/tmp/fatbin-%d.cubin", ava_metadata(NULL)->num_fatbins);
    ...
    if (ava_metadata(NULL)->fd_functions != 0) {
        size = 0;
        write(ava_metadata(NULL)->fd_functions, (const void *)&size, sizeof(size_t));
    }
    ...
    sprintf(function_arg_filename, "/tmp/function_arg-%d.cubin", ava_metadata(NULL)->num_fatbins);
    ...
    while (fgets(line, sizeof(line), fp_pipe) != NULL) {
        ...
        if (strncmp(line, ".nv.info._Z", 11) == 0) {
            ...
            size = strlen(name) + 1;
            write(function_arg_fd, (void *)&size, sizeof(size_t));
            write(function_arg_fd, (void *)name, size);
            write(function_arg_fd, (void *)func, sizeof(struct fatbin_function));
...
ava_utility void __helper_dump_cuda_function(
  ...
  const char    *deviceName,
    ...
    if (fd == 0) {
        fd = open("/tmp/fatfunction.cufunc", O_WRONLY | O_TRUNC | O_CREAT, 0666);
